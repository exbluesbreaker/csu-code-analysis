...
if(args.type=="ClassIR"):
    project = config_parser.get(args.type,'project')
    criteria = config_parser.get(args.type,'criteria')
    out_file = config_parser.get(args.type,'out_file')
    treshold = config_parser.getfloat(args.type,'treshold')
    add_value = config_parser.getboolean(args.type,'add_value')
    ...
    in_file = config_parser.get(args.type,'in_file')
    ...
    project = config_parser.get(args.type,'project')
    in_file = config_parser.get(args.type,'in_file')
    preload_file = config_parser.get(args.type,'preload_file')
    only_preload = config_parser.getboolean(args.type,'only_preload')
    sys.argv = ["main.py",project]
    ...
    in_file = config_parser.get(args.type,'in_file')
    preload_file = config_parser.get(args.type,'preload_file')
    work_dir = config_parser.get(args.type,'work_dir')
    repo = config_parser.get(args.type,'repo')
    only_preload = config_parser.getboolean(args.type,'only_preload')
    ...
    ucr_xml = config_parser.get(args.type,'ucr_xml')
    cfg_xml = config_parser.get(args.type,'cfg_xml')
    out_xml = config_parser.get(args.type,'out_xml')
    runner = DataflowLinker(ucr_xml,cfg_xml,out_xml)
elif(args.type=="CFGVisualizer"):
    lcfg_xml = config_parser.get(args.type,'linked_cfg_xml')
    out_dir = config_parser.get(args.type,'out_dir')
    runner = CFGVisualizer(lcfg_xml,out_dir)
elif(args.type=="InheritanceSlicer"):
    in_file = config_parser.get(args.type,'in_file')
    out_file = config_parser.get(args.type,'out_file')
    class_id = config_parser.get(args.type,'id')
    runner = InheritanceSlicer(in_file,out_file,class_id)
elif(args.type=="FlatCFGSlicer"):
    in_file = config_parser.get(args.type,'in_file')
    out_file = config_parser.get(args.type,'out_file')
    target_id = config_parser.get(args.type,'id')
    criteria = config_parser.get(args.type,'criteria')
    runner = FlatCFGSlicer(in_file,out_file,target_id,criteria)
elif(args.type=="ClassCFGSlicer"):
    in_file = config_parser.get(args.type,'in_file')
    out_file = config_parser.get(args.type,'out_file')
    ucr_id = config_parser.get(args.type,'ucr_id')
    criteria = config_parser.get(args.type,'criteria')
    runner = ClassCFGSlicer(in_file,out_file,ucr_id,criteria)
elif(args.type=="InstanceInitSlicer"):
    ucr_xml = config_parser.get(args.type,'ucr_xml')
    lcfg_xml = config_parser.get(args.type,'lcfg_xml')
    ucr_id = config_parser.get(args.type,'ucr_id')
    out_xml = config_parser.get(args.type,'out_xml')
    keep_parents = config_parser.getboolean(args.type,'keep_parents')
    runner = InstanceInitSlicer(ucr_xml,lcfg_xml,ucr_id,out_xml,keep_parents)
elif(args.type=="ExecPathHandler"):
    in_file = config_parser.get(args.type,'in_file')
    out_file = config_parser.get(args.type,'out_file')
    exec_path = config_parser.get(args.type,'exec_path')
